...
public class PlacementDAOImpl implements PlacementDAO {
    ...
    public List<Object[]> getAppliedStudentsData()
    {
        ...
        try {
            ...
            Query query = session.createSQLQuery("select distinct s.roll_num,s.first_name,s.email,s.cgpa,s.Specialization,ps.Status,s.domain,ps.Placement_id from Placement_Student ps, Students s where ps.Stud_id=s.student_id and ps.Status=\"PENDING\" and s.placement_status_id=0");

            appliedStud = query.list();

            System.out.println("in dao implem");
            System.out.println(appliedStud);
    ...
    public List<Object[]> getEligibleStudentsData()
    {
        ...
        try {
            ...
            Query query = session.createSQLQuery("select distinct s.roll_num,s.first_name,s.email,s.cgpa,s.specialization,s.domain,p.PlacId from Students s, Placement p left outer join Placement_Filter pf on p.PlacId=pf.Placement_id where (p.min_cgpa is null or s.cgpa>=p.min_cgpa) and (pf.domain is null or s.domain=pf.domain) and (pf.specialization is null or s.specialization=pf.specialization) and s.placement_status_id=0");
